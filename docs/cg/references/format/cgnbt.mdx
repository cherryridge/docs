...
If the `Array` contains `Array` type, every tags in `entries` may contain `head` and `payload` data block. The **Type Identifier** (`1000`) is ignored. The **Second Type** and the `payload` data block is parsed according to [`Array` type](#array-type).
