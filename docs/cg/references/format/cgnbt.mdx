...
If the Array contains `Object`, `IVarInt`, `UVarInt`, `Float`, `Double`, `String`, or `Raw` type, every tags in `entries` may contain only `payload` data block and is parsed according to their respective specifications.
