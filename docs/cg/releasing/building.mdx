...
CherryGrove currently only uses CMake. We use the following dependency management strategies:
...
Compiler: **It's recommended to use clang/LLVM-build toolchain on all platforms for better dead code eliminating and unified modern C++ support.**

Linker: Use `lld-link` on Linux and macOS,

Prebuilt dependencies assume linkage against the platform's default C++ Standard Library:

- Windows: Microsoft STL
- Linux: libstdc++
- macOS: libc++
...
Use Microsoft Visual Studio or VS Code to build CherryGrove on Windows.
...
Switch to **Individual Components** tab, and check the following components for the **clang/LLVM toolchain**:
...
I f that package name is not available on your release, try using `7zip-standalone`. Check if 7z is installed using:
...
</TabItem><TabItem value="make" label="Make/GMake">
...
</TabItem><TabItem value="ninja" label="Ninja">
