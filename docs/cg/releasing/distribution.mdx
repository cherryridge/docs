...
Currently we are using the Inno Setup installer, which is an instanced installer and users can choose from a normal and a portable installation.
...
Note that AppImage itself treats every AppImage identical file as a single, global instance, regardless of their location or name. So directly running AppImages without extraction won't enable multi-instancing. It's a limitation, but more like a intended feature. We're simply too special.
...
Note that CherryGrove don't support x86 or armv7a architectures. See [supported platforms](../overview#supported-platforms). There's just no row for them.
...
The most easy way for multi-instancing is the green software approach, i.e. a compressed archive that users can decompress to a directory and run the executable directly. Another way is the "instanced installer", which can either be a self-extracting archive or a installer that allows users to choose the installation directory and disable registry entries and shortcuts, or an APK with a different package name on Android. Provided that users can run executables in custom directories, it would be a good way for now, but it's a luxury to do so on some platforms.

But theoretically the best way is an instance manager like MultiMC or the official Minecraft Launcher. We plan to develop one called [Beehive](https://github.com/cherryridge/Beehive) in the future. It's basically MultiMC but for CherryGrove that allows users to install and manage multiple instances of CherryGrove. It is currently in early development planning. Don't expect it to be available soon.

## Platform-Agnostic Distribution

### Beehive (which itself is still platform-specific)
